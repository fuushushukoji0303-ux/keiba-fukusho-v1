...
def evaluate(horses, remaining):
    ...
    if remaining<100: grade="見送り"
    elif score>=88 and best["confidence"]>=85 and best["place_low"]>=1.3 and best["spread"]<=0.35: grade="S"
    elif score>=80 and best["confidence"]>=78 and best["place_low"]>=1.2: grade="A"
    elif score>=70 and best["confidence"]>=68: grade="B"
    else: grade="見送り"
    reasons=[f"候補評価：{best['confidence']}点",f"優先度：{best['priority_score']:.1f}",f"複勝オッズ：{best['place_low']:.1f}～{best['place_high']:.1f}倍",f"参考EV：{best['ev_index']:.2f}",f"単勝人気順位：{best['market_rank']}位"]
    return {"grade":grade,"score":score,"recs":ranked[:5],"reasons":reasons}


def recommended_amount(grade, remaining, low):
    base={"S":1000,"A":700,"B":300}.get(grade,0)
    if low<1.2: base=0
    return min(base, int(remaining//100)*100)
...
def home():
    ...
    draft=''
    if d:
        draft=f'''<div class="card"><div class="title">現在の複勝候補</div><div class="horse-card"><div class="horse-no">{d.get('horse_no','')}番</div><div class="horse-name">{html.escape(str(d.get('horse_name','')))}</div><div class="pick-grid"><div><span>複勝オッズ</span><strong>{float(d.get('place_low') or 0):.1f}～{float(d.get('place_high') or 0):.1f}倍</strong></div><div><span>判定</span><strong>{html.escape(str(d.get('grade','')))}</strong></div><div><span>参考EV</span><strong>{float(d.get('ev_index') or 0):.2f}</strong></div><div><span>推奨額</span><strong>{int(d.get('amount') or 0):,}円</strong></div></div></div><form method="post" action="/record"><button class="green">この候補を購入記録へ</button></form></div>'''
...
def analyze():
    ...
    form=f'''<div class="card"><div class="title">複勝オッズ取得 → 1頭予想</div><form method="post"><div class="two"><div><label>競馬場</label><select name="course"><option value="">選択</option>{opts}</select></div><div><label>レース</label><select name="race"><option value="">選択</option>{ropts}</select></div></div><br><button class="green">複勝候補を分析</button></form></div>'''
...
def apply():
    ...
    return redirect(url_for("home",msg="1位候補をホームへ入力しました。"))

@app.post("/record")
def record():
    d=get_draft()
    if not d: return redirect(url_for("home",msg="先に複勝候補を分析してください。"))
